...
class broadNet(object):
	...
	def fit(self, data, label):
		...
		label = self.onehotencoder.fit_transform(np.asarray(np.mat(label).T))
	...
	def predict_proba(self, testdata):
		testdata = self.normalscaler.transform(testdata)
		test_inputdata = self.transform(testdata)
		predict_prob = torch.tensor(test_inputdata.dot(self.W))
		predict_prob = self.softmax(predict_prob)
		return predict_prob.numpy()
...
if __name__ == '__main__':
    ...
    test_data = pd.read_csv('./pre_test_titanic.csv')
    samples_data = pd.read_csv('./titanic/gender_submission.csv')
	# label 做标准化
    # label 做标准化
    ...
    for item in test_data.columns:
        test_data[item] = le.fit_transform(test_data[item])
    ...
    # save as csv file
    samples = samples_data['PassengerId']
    result = {'PassengerId':samples,
              'Survived': predicts }
    result = pd.DataFrame(result)

    result.to_csv('./titanic_bls_addenhancenodes.csv', index=False)
